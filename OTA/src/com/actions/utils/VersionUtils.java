...
Log.d(TAG, "CheckVersion mUpdateInfoContentHandler=" + mUpdateInfoContentHandler);
        if (mUpdateInfoContentHandler != null){
            ...
            if ((targetUpdateInfo_full == null) && (targetUpdateInfo_diff == null)) {
                // no update
                Log.d(TAG, "yison CheckVersion targetUpdateInfo_full=" + targetUpdateInfo_full);
                Log.d(TAG, "yison CheckVersion targetUpdateInfo_diff=" + targetUpdateInfo_diff);
            } else {
                if ((targetUpdateInfo_full != null) && (targetUpdateInfo_diff == null)) {
    ...
    private UpdateInfo getTargetFullUpdateInfo(List<UpdateInfo> mUpdateInfoList) {
        ...
        // }
        if (ACTDEBUG) {
            Log.d(TAG, "yison getTargetFullUpdateInfo : ");

            if (targetUpdateInfo == null) {
                Log.d(TAG, "yison getTargetFullUpdateInfo null ");
            } else {
	...
	if (ACTDEBUG) {
        Log.d(TAG, " yison getTargetDiffUpdateInfo: ");
		if (targetUpdateInfo == null) {
			Log.d(TAG, " yison targetUpdateInfo is null ");
		} else {
			printUpdateInfo(targetUpdateInfo);
		}
	}
    ...
    public void printUpdateInfo(UpdateInfo mInfo) {
        if (mInfo == null)
            return;
        Log.d(TAG, " yison UpdateType :" + mInfo.getUpdateType());
        Log.d(TAG, " yison OldVersion :" + mInfo.getOldVersion());
        Log.d(TAG, " yison getNewVersion :" + mInfo.getNewVersion());
        Log.d(TAG, " yison DownloadUrl :" + mInfo.getDownloadUrl());
        Log.d(TAG, " yison FileSize :" + mInfo.getFileSize());
        Log.d(TAG, " yison Md5 :" + mInfo.getMd5());
    }
